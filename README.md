...
- *mu-dev* , a cargo-like development and packaging utility
- *mu-repl* , a configurable interactive tool for exploration and debugging 
...
*mu*, *utility*, and *core* reference cards can be found in ```doc/refcards``` in a variety of formats. They document the *mu*  and *core* namespaces, the runtime API, and options for running *mu-sys*, *mu-repl*, and *mu-dev*.
...
After cloning the *mu* repository, the system can be built and installed with the supplied makefile. The *world* target builds a release version of the system and the *mu-dev* development tool.  `make` with no arguments prints the available targets. 
...
Having built and installed `mu`,  establish the current directory as a `mu-dev`  workspace.

```
% mu-dev workspace init
```
...
The *mu-dev* command is found at `/opt/mu/bin/mu-dev`.

```
Usage: mu-dev 0.0.21 command [option...]
  command:
    help                               ; this message
    version                            ; mu-dev version
...
`mu-dev` is styled after `cargo` and fulfills many of the same functions. While the help message should be relatively explanatory, the general development workflow is something like this. Note that in this version **=** is mandatory for options with arguments.

Before making any changes, you should establish a performance baseline.

```
 mu-dev bench base [--ntests=50]
```
...
```
 ...
 mu-dev regression                     # run the regression tests
 mu-dev bench current [--ntests=50]    # bench current build
 mu-dev bench report                   # print metric changes
```
...
The distribution includes a command line tool for running and interacting with the system. The *mu-repl* binary is part of the release, found at `/opt/mu/bin/mu-repl`.

*mu-repl* has no command line arguments. It is configured by an optional JSON file, *.mu-replrc*, which is expected to be in either the current directory or the user's home directory. The *config* argument supplies a *mu* environment configuration string (see **System Configuration** for details), and the *load* argument supplies a list of the names of files to load on startup using the *loader* function. The *reader* argument, currently limited to `core` or `mu` (the default) tells *mu-repl* to run in the indicated namespace and use the associated reader. *require* is a list of modules to be loaded by *core:require*, and *lib* is a list of *.sys* modules to be loaded from the release *lib* directory. *options* contains a list of *mu-repl* options (of which there are none as of 0.0.5). All arguments are optional.  
...
```
% mu-dev bench base [--ntests=50]
% mu-dev bench current [--ntests=50]
% mu-dev bench report
```
...
```
/opt/mu/bin
├── mu-dev		# development tool
├── mu-repl		# runtime binary, stdio repl
