...
- *mforge* , a cargo-like development and packaging tool
...
*mu*, *utility*, and *core* reference cards can be found in ```doc/refcards``` in a variety of formats. They document the *mu*  and *core* namespaces, the runtime API, and options for running *mu-sys*, *listener*, and *mforge*.
...
After cloning the *mu* repository, the *mu* system can be built and installed with the supplied makefile. The *world* target builds a release version of the system and the *mforge* development tool.  `make` with no arguments prints the available targets. 
...
Having built and installed `mu`,  establish the current directory as a `mforge`  workspace.

```
% mforge workspace init
```

Subsequent builds and packaging of the system are facilitated with *mforge*. See the *Tools* section below for usage instructions.

Note: the *mforge* and *makefile* installation mechanisms do not remove the installation directory before writing it and changes to directory structure and files will accumulate.
...
The  *mforge* command is part of a release, found at `/opt/mu/bin/mforge`.

```
Usage: mforge 0.0.18 command [option...]
  command:
    help                               ; this message
    version                            ; mforge version
...
`mforge` is styled after `cargo` and fulfills many of the same needs. While the help message should be relatively explanatory, the general development workflow is something like this. Note that in this version **=** is mandatory for options with arguments.

Before making any changes, you will want to establish a performance baseline.

```
 mforge bench base [--ntests=50]
```
...
```
 mforge build release				# build the release version 
 mforge regression					# run the regression tests
 mforge bench current [--ntests=50]	# bench current build
 mforge bench report [--all]			# print
```
...
```
% mforge bench base [--ntests=50]
% mforge bench current [--ntests=50]
% mforge bench report [--all]
```
...
├── mu-server	# runtime binary, socket listener
