...
- introducing the *mux* tool

- uncouple *mux* from the distribution

- simplify initial project build

- deprecate the current root makefile
...
A handy *mu* reference card can be found in ```doc/refcards``` in a variety of formats. It documents the *mu* namespace, the runtime API, and options for running *mu-sys*.
...
Having built the distribution, install it in `/opt/mu`. You can change where the *mux* tool installs by altering the `MUX_DIR` environment variable. 

`MUX_DIR` defaults to `/usr/local/bin`.
