...
*mu/sys* should build with rust 1.70 or better. *mu/sys* builds are targeted to:
...
A handy ```mu/sys``` reference card can be found in ```doc/refcards``` in a variety of formats.

The `mu/sys` crate rustdoc documentation can be generated by
...
#### Building and installing the *mu* system
...
Failures in the *mu/sys* tests are almost guaranteed to cause complete failure of subsequent tests.
...
An interactive session for the extended *mu* system is invoked by the`mu` shell script, `:h` will print the currently available repl commands. Forms entered at the prompt are evaluated and the results printed. The prompt displays the current namespace.
