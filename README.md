...
A handy *mu-sys* reference card can be found in ```doc/refcards``` in a variety of formats. It documents the *lib* namespace, the runtime API, and options for running *mu-sys*.
...
The *lib* runtime is a native code program that must be built for the target CPU architecture. The *mu-sys* build system requires only a `rust` compiler, `rust-fmt`, `clippy` and the  GNU `make` utility. Other tools like  `valgrind` are optional.
