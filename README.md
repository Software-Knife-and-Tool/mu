...
The runtime builds with rust 1.70 or better. *mu-sys* builds are targeted to:
...
The *mu* runtime kernel, *libcore*, is written in mostly-safe `rust` (the system image/heap facility *mmaps* a file, which is an inherently unsafe operation.)
...
A handy *mu-sys* reference card can be found in ```doc/refcards``` in a variety of formats. It documents the *libcore* namespace, the runtime API, and options for running *mu-sys*.

The *libcore* crate rustdoc documentation can be generated by
...
The *libcore* runtime is a native code program that must be built for the target CPU architecture. The *mu-sys* build system requires only a `rust` compiler, `rust-fmt`, `clippy` and the  GNU `make` utility. Other tools like  `valgrind` are optional.
...
As of 0.0.40, the *libcore* runtime supports conditional compilation of a variety of features. 
...
Failures in the *libcore* tests are almost guaranteed to cause complete failure of subsequent tests.
