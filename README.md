...
- *mu-runtime*, a small, configurable runtime library amd language
...
The *mu* runtime kernel, *mu-runtime*, is written in mostly-safe `rust` (the system image/heap facility *mmaps* a file and random user selected features may have unsafe implementations.)
...
*mu-rutime*  and *core* reference cards can be found in ```doc/refcards``` in a variety of formats. They document the *mu*  and *core* namespaces, the runtime API, and options for running *mu-sys*.
