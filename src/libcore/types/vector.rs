        ...
        match syntax {
            ...
            '(' => {
                ...
                match VTYPEMAP.iter().copied().find(|tab| vec_type.eq_(&tab.0)) {
                    ...
                    None => Err(Exception::new(env, Condition::Type, "core:read", vec_type)),
                }
            }
            _ => panic!(),
        }
...
pub trait CoreFunction {
    fn core_type(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn core_length(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn core_make_vector(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn core_svref(_: &Env, _: &mut Frame) -> exception::Result<()>;
}

impl CoreFunction for Vector {
    fn core_make_vector(env: &Env, fp: &mut Frame) -> exception::Result<()> {
                    ...
                    let vec: exception::Result<String> = Cons::iter(env, list)
                        .map(|cons| {
                            ...
                            if ch.type_of() == Type::Char {
                                ...
                                Err(Exception::new(env, Condition::Type, "core:make-vector", ch))
                            }
                        })
                        .collect();
                    ...
                    let vec: exception::Result<Vec<u8>> = Cons::iter(env, list)
                        .map(|cons| {
                            ...
                            if fx.type_of() == Type::Fixnum {
                                ...
                                Err(Exception::new(env, Condition::Type, "core:make-vector", fx))
                            }
                        })
                        .collect();
                    ...
                    let vec: exception::Result<Vec<f32>> = Cons::iter(env, list)
                        .map(|cons| {
                            ...
                            if fl.type_of() == Type::Float {
                                ...
                                Err(Exception::new(env, Condition::Type, "core:make-vector", fl))
                            }
                        })
                        .collect();
    ...
    fn core_svref(env: &Env, fp: &mut Frame) -> exception::Result<()> {
