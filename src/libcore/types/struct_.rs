...
pub trait CoreFunction {
    fn core_struct_type(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn core_struct_vector(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn core_make_struct(_: &Env, _: &mut Frame) -> exception::Result<()>;
}
...
impl CoreFunction for Struct {
    ...
    fn core_make_struct(env: &Env, fp: &mut Frame) -> exception::Result<()> {
        let stype = fp.argv[0];
        let list = fp.argv[1];
