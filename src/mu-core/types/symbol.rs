...
pub trait CoreFunction {
    fn core_name(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn core_ns(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn core_value(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn core_boundp(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn core_symbol(_: &Env, _: &mut Frame) -> exception::Result<()>;
}

impl CoreFunction for Symbol {
    fn core_name(env: &Env, fp: &mut Frame) -> exception::Result<()> {
    ...
    fn core_ns(env: &Env, fp: &mut Frame) -> exception::Result<()> {
        let symbol = fp.argv[0];
    ...
    fn core_value(env: &Env, fp: &mut Frame) -> exception::Result<()> {
        let symbol = fp.argv[0];
    ...
    fn core_boundp(env: &Env, fp: &mut Frame) -> exception::Result<()> {
        let symbol = fp.argv[0];
        ...
        fp.value = match symbol.type_of() {
            ...
            _ => return Err(Exception::new(env, Condition::Type, "core:boundp", symbol)),
        };
