...
impl Core for Symbol {
    ...
    fn mark(mu: &Mu, symbol: Tag) {
        match symbol {
            Tag::Direct(_) => (), // keyword
            Tag::Indirect(_) => {
                let mark = mu.mark_image(symbol).unwrap();

                if !mark {
                    mu.mark(Self::name(mu, symbol));
                    mu.mark(Self::value(mu, symbol));
                }
            }
        }
    }
            ...
            Type::Symbol => {
                ...
                if escape {
                    ...
                    if !Tag::null_(&ns) && !mu.null_ns.eq_(&ns) {
                        match mu.write_stream(Symbol::name(mu, ns), false, stream) {
                            Ok(_) => (),
                            Err(e) => return Err(e),
                        }
                ...
                mu.write_stream(name, false, stream)
            }
            _ => panic!(),
        }
    }
...
pub trait MuFunction {
    ...
    fn lib_boundp(_: &Mu, _: &mut Frame) -> exception::Result<()>;
    fn lib_symbol(_: &Mu, _: &mut Frame) -> exception::Result<()>;
...
impl MuFunction for Symbol {
    ...
    fn lib_ns(mu: &Mu, fp: &mut Frame) -> exception::Result<()> {
        let symbol = fp.argv[0];
    ...
    fn lib_value(mu: &Mu, fp: &mut Frame) -> exception::Result<()> {
        let symbol = fp.argv[0];
    ...
    fn lib_boundp(mu: &Mu, fp: &mut Frame) -> exception::Result<()> {
        let symbol = fp.argv[0];
    ...
    fn lib_keyword(mu: &Mu, fp: &mut Frame) -> exception::Result<()> {
        let symbol = fp.argv[0];
    ...
    fn lib_symbol(mu: &Mu, fp: &mut Frame) -> exception::Result<()> {
        let name = fp.argv[0];
