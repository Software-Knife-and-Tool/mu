...
impl Core for Mu {
    fn new(config: &Config) -> Self {
        ...
        mu.lib_ns = Symbol::keyword("lib");
        match Namespace::add_ns(&mu, mu.lib_ns) {
            Ok(_) => (),
            Err(_) => panic!(),
        };
        ...
        Namespace::intern_symbol(&mu, mu.lib_ns, "std-in".to_string(), mu.stdin);
        Namespace::intern_symbol(&mu, mu.lib_ns, "std-out".to_string(), mu.stdout);
