...
pub trait CoreFunction {
    fn crux_future_defer(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn crux_future_detach(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn crux_future_force(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn crux_future_poll(_: &Env, _: &mut Frame) -> exception::Result<()>;
}

impl CoreFunction for Future {
    fn crux_future_force(env: &Env, fp: &mut Frame) -> exception::Result<()> {
        let future = fp.argv[0];

        env.fp_argv_check("crux:force", &[Type::Struct], fp)?;
        ...
        fp.value = if Struct::stype(env, future).eq_(&Symbol::keyword("future")) {
            ...
            return Err(Exception::new(env, Condition::Type, "crux:force", future));
        };

        Ok(())
    }

    fn crux_future_poll(env: &Env, fp: &mut Frame) -> exception::Result<()> {
        let future = fp.argv[0];

        env.fp_argv_check("crux:poll", &[Type::Struct], fp)?;
        fp.value = if Struct::stype(env, future).eq_(&Symbol::keyword("future")) {
            ...
            return Err(Exception::new(env, Condition::Type, "crux:poll", future));
        };

        Ok(())
    }

    fn crux_future_defer(env: &Env, fp: &mut Frame) -> exception::Result<()> {
        let func = fp.argv[0];
        let args = fp.argv[1];

        env.fp_argv_check("crux:defer", &[Type::Function, Type::List], fp)?;
    ...
    fn crux_future_detach(env: &Env, fp: &mut Frame) -> exception::Result<()> {
        let func = fp.argv[0];
        let args = fp.argv[1];

        env.fp_argv_check("crux:detach", &[Type::Function, Type::List], fp)?;
