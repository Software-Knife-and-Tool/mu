    ...
    fn read_comment(env: &Env, stream: Tag) -> exception::Result<Option<()>> {
        loop {
            match Stream::read_char(env, stream)? {
                ...
                None => return Err(Exception::new(env, Condition::Eof, "crux:read", stream)),
            }
        }
        ...
        while let Some(ch) = Stream::read_char(env, stream)? {
            match map_char_syntax(ch) {
                Some(stype) => match stype {
                    ...
                    _ => return Err(Exception::new(env, Condition::Range, "crux:read", stream)),
                },
                None => return Err(Exception::new(env, Condition::Range, "crux:read", stream)),
            }
        }
                ...
                ':' => match Stream::read_char(env, stream)? {
                    Some(ch) => {
                        ...
                        match atom.type_of() {
                            ...
                            _ => Err(Exception::new(env, Condition::Type, "crux:read", stream)),
                        }
                    }
                    None => Err(Exception::new(env, Condition::Eof, "crux:read", stream)),
                },
