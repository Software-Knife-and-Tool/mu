...
pub trait CoreFunction {
    fn crux_unwind_protect(env: &Env, fp: &mut Frame) -> Result<()>;
    fn crux_raise(env: &Env, fp: &mut Frame) -> Result<()>;
}
...
impl CoreFunction for Exception {
    fn crux_raise(env: &Env, fp: &mut Frame) -> Result<()> {
        ...
        env.fp_argv_check("crux:raise", &[Type::T, Type::Keyword], fp)?;
        match Self::map_condition(env, condition) {
            Ok(cond) => Err(Self::new(env, cond, "crux:raise", src)),
            Err(_) => Err(Self::new(env, Condition::Type, "crux:raise", condition)),
        }
    }
    ...
    fn crux_unwind_protect(env: &Env, fp: &mut Frame) -> Result<()> {
        ...
        env.fp_argv_check("crux:unwind-protect", &[Type::Function, Type::Function], fp)?;
