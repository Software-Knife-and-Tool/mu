        ...
        match func.type_of() {
            Type::Symbol => {
                if Symbol::is_bound(env, func) {
                    ...
                    Err(Exception::new(env, Condition::Unbound, "crux:apply", func))
                }
            }
            Type::Function => match Function::form(env, func).type_of() {
                ...
                Type::Vector => {
                    ...
                    if nargs != nreqs {
                        return Err(Exception::new(env, Condition::Arity, "crux:apply", func));
                    }
                ...
                _ => Err(Exception::new(env, Condition::Type, "crux:apply", func)),
            },
            _ => Err(Exception::new(env, Condition::Type, "crux:apply", func)),
        }
    }
...
pub trait CoreFunction {
    fn crux_fr_pop(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn crux_fr_push(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn crux_fr_ref(_: &Env, _: &mut Frame) -> exception::Result<()>;
}

impl CoreFunction for Frame {
    fn crux_fr_pop(env: &Env, fp: &mut Frame) -> exception::Result<()> {
        fp.value = fp.argv[0];

        env.fp_argv_check("crux:frame-pop", &[Type::Function], fp)?;
    ...
    fn crux_fr_push(env: &Env, fp: &mut Frame) -> exception::Result<()> {
        fp.value = fp.argv[0];

        env.fp_argv_check("crux:frame-push", &[Type::Vector], fp)?;
    ...
    fn crux_fr_ref(env: &Env, fp: &mut Frame) -> exception::Result<()> {
        let frame = fp.argv[0];
        let offset = fp.argv[1];

        env.fp_argv_check("crux:frame-ref", &[Type::Fixnum, Type::Fixnum], fp)?;
