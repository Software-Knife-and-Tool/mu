...
pub trait CoreFunction {
    fn crux_name(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn crux_ns(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn crux_value(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn crux_boundp(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn crux_symbol(_: &Env, _: &mut Frame) -> exception::Result<()>;
}

impl CoreFunction for Symbol {
    fn crux_name(env: &Env, fp: &mut Frame) -> exception::Result<()> {
    ...
    fn crux_ns(env: &Env, fp: &mut Frame) -> exception::Result<()> {
        let symbol = fp.argv[0];
    ...
    fn crux_value(env: &Env, fp: &mut Frame) -> exception::Result<()> {
        let symbol = fp.argv[0];
    ...
    fn crux_boundp(env: &Env, fp: &mut Frame) -> exception::Result<()> {
        let symbol = fp.argv[0];
        ...
        fp.value = match symbol.type_of() {
            ...
            _ => return Err(Exception::new(env, Condition::Type, "crux:boundp", symbol)),
        };

        Ok(())
    }

    fn crux_symbol(env: &Env, fp: &mut Frame) -> exception::Result<()> {
        let name = fp.argv[0];

        env.fp_argv_check("crux:make-symbol", &[Type::String], fp)?;
