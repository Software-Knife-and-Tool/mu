                ...
                Type::Symbol if dot.eq_(&Symbol::name(env, car)) => {
                    ...
                    if EOL.eq_(&cdr) {
                        ...
                        if EOL.eq_(&eol) {
                            ...
                            Err(Exception::new(env, Condition::Eof, "crux:read", stream))
                        }
                    }
                }
                _ => {
...
pub trait CoreFunction {
    fn crux_append(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn crux_car(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn crux_cdr(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn crux_cons(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn crux_length(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn crux_nth(_: &Env, _: &mut Frame) -> exception::Result<()>;
    fn crux_nthcdr(_: &Env, _: &mut Frame) -> exception::Result<()>;
}

impl CoreFunction for Cons {
    fn crux_append(env: &Env, fp: &mut Frame) -> exception::Result<()> {
        let list1 = fp.argv[0];
        let list2 = fp.argv[1];
    ...
    fn crux_car(env: &Env, fp: &mut Frame) -> exception::Result<()> {
        let list = fp.argv[0];

        env.fp_argv_check("crux:car", &[Type::List], fp)?;
    ...
    fn crux_cdr(env: &Env, fp: &mut Frame) -> exception::Result<()> {
        let list = fp.argv[0];

        env.fp_argv_check("crux:cdr", &[Type::List], fp)?;
        fp.value = match list.type_of() {
            Type::Null => list,
    ...
    fn crux_nth(env: &Env, fp: &mut Frame) -> exception::Result<()> {
        let nth = fp.argv[0];
        let list = fp.argv[1];

        env.fp_argv_check("crux:nth", &[Type::Fixnum, Type::List], fp)?;

        if Fixnum::as_i64(nth) < 0 {
            return Err(Exception::new(env, Condition::Type, "crux:nth", nth));
        }
        ...
        fp.value = match list.type_of() {
            ...
            Type::Cons => match Self::nth(env, Fixnum::as_i64(nth) as usize, list) {
                Some(tag) => tag,
                None => return Err(Exception::new(env, Condition::Type, "crux:nth", list)),
            },
            _ => panic!(),
        };
    ...
    fn crux_nthcdr(env: &Env, fp: &mut Frame) -> exception::Result<()> {
        let nth = fp.argv[0];
        let list = fp.argv[1];

        env.fp_argv_check("crux:nthcdr", &[Type::Fixnum, Type::List], fp)?;

        if Fixnum::as_i64(nth) < 0 {
            return Err(Exception::new(env, Condition::Type, "crux:nthcdr", nth));
        }
        ...
        fp.value = match list.type_of() {
            ...
            Type::Cons => match Self::nthcdr(env, Fixnum::as_i64(nth) as usize, list) {
                Some(tag) => tag,
                None => return Err(Exception::new(env, Condition::Type, "crux:nthcdr", list)),
            },
            _ => panic!(),
        };
