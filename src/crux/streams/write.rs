...
pub trait CoreFunction {
    fn crux_write(_: &Env, _: &mut Frame) -> exception::Result<()>;
}

impl CoreFunction for Env {
    fn crux_write(env: &Env, fp: &mut Frame) -> exception::Result<()> {
        fp.value = fp.argv[0];
        let escape = fp.argv[1];
        let stream = fp.argv[2];

        env.fp_argv_check("crux:write", &[Type::T, Type::T, Type::Stream], fp)?;
