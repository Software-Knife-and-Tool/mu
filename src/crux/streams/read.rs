    ...
    fn read_stream(
        ...
        if Lib::read_ws(self, stream)?.is_none() {
            return if eof_error_p {
                Err(Exception::new(self, Condition::Eof, "crux:read", stream))
            } else {
                Ok(eof_value)
            };
        };
                    ...
                    SyntaxType::Tmacro => match ch {
                        ...
                        ')' => {
                            if recursivep {
                                ...
                                Err(Exception::new(self, Condition::Syntax, "crux:read", stream))
                            }
                        }
                        ';' => {
...
pub trait CoreFunction {
    fn crux_read(_: &Env, _: &mut Frame) -> exception::Result<()>;
}

impl CoreFunction for Env {
    fn crux_read(env: &Env, fp: &mut Frame) -> exception::Result<()> {
        let stream = fp.argv[0];
        let eof_error_p = fp.argv[1];
        let eof_value = fp.argv[2];

        env.fp_argv_check("crux:read", &[Type::Stream], fp)?;
