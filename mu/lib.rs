...
//!
//! The mu-mu machine is the implementation surface for the [`mu programming environment`].
//!
//! As much as is practible, mu-mu's functions and data types resemble Common Lisp in preference to
//! Scheme/Clojure in order to be immediately familiar to the traditional LISP programmer.
//!
//! mu-mu is an immutable, lexically scoped LISP-1 kernel meant as a porting layer for an ascending
//! tower of LISP languages. While it is possible to do some useful application work directly in the
//! mu-mu language, mu-mu defers niceties like macros, closures, and rest functions to a compiler
//! layered on it. See [`mu programming environment`] for details.
//!
//! mu-mu characteristics:
//! - mostly-safe Rust
...
/// The mu-mu API
///
/// The mu-mu API exposes these types:
...
impl Mu {
    ...
    pub fn compile(&self, expr: Tag) -> exception::Result<Tag> {
        Compiler::compile(&self.0, expr, &mut vec![])
    }
